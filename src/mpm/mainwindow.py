...
class Window:
    def __init__(self, title, icon_path):
        ...
        self.ui = mpm.mainwindow_ui.Ui_MainWindow()
        ...
        self.uuid_notifiers = {
            "can": mpm.canmodel.ReferencedUuidNotifier(),
            "staticmodbus": mpm.staticmodbusmodel.ReferencedUuidNotifier(),
            "check_result": epyqlib.checkresultmodel.ReferencedUuidNotifier(),
            "sunspec1": mpm.sunspecmodel.ReferencedUuidNotifier(),
            "sunspec2": mpm.sunspecmodel.ReferencedUuidNotifier(),
        }
        ...
        search_boxes = (
            ...
            (
                ...
                mpm.canmodel.columns.index_of("Name"),
            ),
            (
                self.ui.static_modbus_view,
                self.ui.static_modbus_search_box,
                mpm.staticmodbusmodel.columns.index_of("Name"),
            ),
            (
            ...
            (
                ...
                mpm.sunspecmodel.columns.index_of("Name"),
            ),
            (
    ...
    def import_smdx(self):
        ...
        for smdx_path in selected:
            ...
            model = mpm.smdxtosunspec.import_model(
                model_id=id,
    ...
    def full_import(self):
        dialog = mpm.importexportdialog.import_dialog()
        if dialog.exec() != QtWidgets.QDialog.DialogCode.Accepted:
            return

        project = mpm.importexport.full_import(paths=dialog.paths_result)
    ...
    def _full_export(self, first_time):
        dialog = mpm.importexportdialog.export_dialog()
        if dialog.exec() != QtWidgets.QDialog.DialogCode.Accepted:
            return
    ...
    def open_project(self, filename=None, project=None):
        if project is not None:
            ...
            if filename is None:
                self.project = mpm.project.create_blank()
            else:
                self.project = mpm.project.loadp(filename)

        model_views = mpm.project.Models()
    ...
    def generate_code(self, node):
        builder = mpm.parameterstoc.builders.wrap(node)
    ...
    def generate_symbol_file(self):
        ...
        builder = mpm.cantosym.builders.wrap(
    ...
    def about_dialog(self):
        message = [
            __copyright__,
            __license__,
            f"Version Tag: {mpm.__version_tag__}",
            f"Commit SHA: {mpm.__sha__}",
            f"Build Tag: {mpm.__build_tag__}",
        ]
