...
def parse_sunspec_fixed_block(sunspec_fixed_block, sunspec_types, staticmodbus_types):
    ...
    for child in sunspec_fixed_block["children"]:
        if child["_type"] == "data_point":
            ...
            stripped_child = {
                key: val for key, val in child.items() if key != "children"
            }
            stripped_child["_type"] = replace_data_point_with_function_data(
                stripped_child["_type"]
            )
...
def parse_sunspec_model(sunspec_model, sunspec_types, staticmodbus_types):
    found_data = []
...
def parse_table(table, sunspec_types, staticmodbus_types):
